...
export const getUserProfile = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const result = await pool.query(
      "SELECT id, email, first_name, last_name, avatar_url, monthly_budget, currency, language, notifications_enabled, created_at, updated_at FROM users WHERE id = $1",
      [req.user!.userId]
    )
...
export const updateUserProfile = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { first_name, last_name, avatar_url, monthly_budget } = req.body
    ...
    const result = await pool.query(
      `UPDATE users
        ...
        WHERE id = $5
        RETURNING id, email, first_name, last_name, avatar_url, monthly_budget, currency, language, notifications_enabled, created_at, updated_at`,
      [first_name, last_name, avatar_url, monthly_budget, req.user!.userId]
    )
