...
export default function BudgetAlert({ totalSpent, budget, stats }: BudgetAlertProps) {
  ...
  if (stats) {
    console.log('🔍 BudgetAlert recibió stats:', stats)
  ...
  if (monthlyBudget <= 0) {
    console.log('ℹ️ BudgetAlert: No hay presupuesto definido')
    return null
  }
  ...
  if (!isWarning) {
    console.log(`ℹ️ BudgetAlert: ${percentage.toFixed(1)}% del presupuesto - Sin alerta`)
    return null
  }

  console.log('✅ BudgetAlert mostrando alerta:', { 
    spent, 
    monthlyBudget, 
    percentage: percentage.toFixed(1),
    tipo: isDanger ? 'PELIGRO' : 'ADVERTENCIA'
  })
